...
public static class Converters
{
    ...
    {
        double result = default(double);
        double.TryParse(data.ToString().Replace(".", ""), out result);
        return Convert.ToDouble(result.ToString("N2"));
    }
    ...
    //public static int ToCheckSum<T>(this List<T> data)
    //{
    //    IEnumerable<int?> value = from c in data select SqlFunctions.Checksum(c.ToString());
    //    if (value != null)
    //        return (from c in value select SqlFunctions.ChecksumAggregate(value)).SingleOrDefault() ?? 0;
    //    return 0;
    //}
